...
function parseRssFeed(xml: string) {
  ...
  const description = decodeXmlEntities(
    extractTagValue(channelXml, "description"),
  );
  ...
  while ((match = itemRegex.exec(channelXml)) !== null) {
    ...
    const itemContent = decodeXmlEntities(
      extractTagValue(itemXml, "content:encoded"),
    );
    const itemDescription = decodeXmlEntities(
      extractTagValue(itemXml, "description"),
    );
    const itemAuthor = decodeXmlEntities(
      extractFirstTagValue(itemXml, ["dc:creator", "author"]),
    );
...
function isAtomFeed(xml: string): boolean {
  return (
    xml.includes('xmlns="http://www.w3.org/2005/Atom"') || xml.includes("<feed")
  );
}
